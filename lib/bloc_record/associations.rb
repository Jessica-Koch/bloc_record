...
module Associations
  def has_many(association)
    define_method(association) do # instance method added to AddressBook
  ...
  def has_one(association)
    define_method(association) do
      ...
      row = self.class.connection.get_first_row <<-SQL
        SELECT * FROM #{association.to_s.name}
        WHERE #{self.class.table}_id = #{self.id}
      SQL
  ...
  def has_many(association)
    define_method(association) do 
      rows = self.class.connection.exectute <<-SQL
        SELECT * FROM #{association.to_s.singularize}
